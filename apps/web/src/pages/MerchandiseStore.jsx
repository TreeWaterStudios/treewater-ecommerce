...
const isValidProductId = value => {
  ...
  return id && lower !== 'undefined' && lower !== 'null' && !lower.includes('undefined') && !lower.includes('null') && !id.startsWith('fallback');
};
const getProductId = (product, index) => {
  const candidates = [product?.sync_product?.id, product?.syncProduct?.id, product?.sync_product_id, product?.syncProductId, product?.sync_variants?.[0]?.sync_product_id, product?.variants?.[0]?.sync_product_id, product?.product?.id, product?.product_id, product?.productId, product?.id];
...
const normalizeProduct = (product, index) => {
  const productVariants = product?.variants || product?.sync_variants || [];
  const firstVariant = productVariants[0] || {};
  const productId = getProductId(product, index);
  const productName = product?.name || product?.title || product?.sync_product?.name || product?.syncProduct?.name || product?.product?.name || 'Unnamed Product';
  const productPrice = product?.price || product?.retail_price || product?.price_in_cents / 100 || firstVariant?.retail_price || firstVariant?.price || 0;
  const productImage = product?.image || product?.thumbnail || product?.thumbnail_url || product?.sync_product?.thumbnail_url || product?.syncProduct?.thumbnail_url || product?.product?.thumbnail_url || firstVariant?.files?.[0]?.preview_url || firstVariant?.files?.[0]?.thumbnail_url || '/images/treewater-hoodie-front.jpg';
  return {
    ...product,
    productId,
    productKey: `merch-product-${productId}-${index}`,
    productName,
    productPrice,
    productImage
  };
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      ...
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        try {
          console.log(
            `[STORE] 🚀 Fetching Printful products — attempt ${attempt} of 3`
          );
          ...
          const normalizedProducts = items
            ...
            .filter(product => {
              ...
              if (
                ...
                console.error(
                  '[STORE] Removed product with bad ID:',
                  product
                );
          ...
          console.log(
            '[STORE] 📊 Total Printful products extracted:',
          ...
          if (normalizedProducts.length > 0) {
            normalizedProducts.forEach(product => {
              console.log(
                `[STORE] 🏷️ Printful Product - ID: ${product.productId}, Name: ${product.productName}`
              );
            });
          ...
          lastError = new Error(
            'The product server returned no usable products.'
          );

          console.warn(
            `[STORE] ⚠️ No usable products returned on attempt ${attempt}`
          );
        } catch (err) {
          lastError = err;

          console.error(
            `[STORE] ❌ Product request failed on attempt ${attempt}:`,
            err
          );
        }
  ...
  return <div className="merch-page-bg min-h-screen bg-[#0a0a0a] flex flex-col font-sans text-gray-100">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 w-full">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            ...
            <p className="text-lg text-gray-400 max-w-2xl leading-relaxed">
              Premium quality apparel and accessories with Free Shipping. Represent with TreeWater, wherever you go.  Sizes may run one size larger for T-Shirts.
            </p>
          </div>
        ...
        {error ? <div className="text-center py-24 bg-red-950/20 rounded-3xl border border-red-900/50">
            ...
            {[...Array(8)].map((_, i) => <div key={`skeleton-${i}`} className="flex flex-col gap-4">
            ...
            {products.map((product, index) => <motion.div key={product.productKey} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.05
                ...
                <Link to={`/product/${encodeURIComponent(product.productId)}`} state={{
            product
          }} className="relative aspect-square rounded-2xl overflow-hidden bg-white/5 border border-white/10 mb-4 block">
                  <img src={product.productImage} alt={product.productName} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onError={e => {
              e.currentTarget.src = '/images/treewater-hoodie-front.jpg';
            }} />
